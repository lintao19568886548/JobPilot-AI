...
$database="jobpilot_phase11_migration_check_$PID"
$port=18091
...
try{
  ...
  $env:MYSQL_DATABASE=$database;$env:SERVER_PORT=[string]$port;$env:BOOTSTRAP_USER_ENABLED='false';$env:AUTOMATION_CENTER_ENABLED='false'
  ...
  $stdout=Join-Path $runtime 'phase11-migration-check.out.log';$stderr=Join-Path $runtime 'phase11-migration-check.err.log'
  ...
  $ready=$false;for($i=0;$i-lt90;$i++){Start-Sleep -Milliseconds 500;try{if((Invoke-RestMethod "http://127.0.0.1:$port/actuator/health" -TimeoutSec 2).status-eq'UP'){$ready=$true;break}}catch{};if($process.HasExited){break}}
  if(!$ready){$tail=(Get-Content $stdout -Tail 40 -ErrorAction SilentlyContinue)-join[Environment]::NewLine;throw "Empty-schema backend did not start. Safe log tail: $tail"}
  ...
  $parts=$result-split',';if($parts.Count-ne3-or$parts[0]-ne'11'-or$parts[1]-ne'11'-or$parts[2]-ne'82'){throw "Unexpected migration result: $result"}
  ...
  if($dbUser){"REVOKE IF EXISTS ALL PRIVILEGES ON $database.* FROM '$dbUser'@'%'; DROP DATABASE IF EXISTS $database;"|docker compose exec -T mysql sh -lc 'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -uroot'|Out-Null}else{"DROP DATABASE IF EXISTS $database;"|docker compose exec -T mysql sh -lc 'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysql -uroot'|Out-Null}
  Pop-Location
}
